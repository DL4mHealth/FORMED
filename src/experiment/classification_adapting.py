...
class ExpConfig:
    ...
    # Model config
    checkpoint_path: str = "./checkpoint/MeTA-{SEED}.pth"
    ...
    def __str__(self) -> str:
        return (
            "classification_attention_finetune-TimesFM"
            f"-ds_{self.dataset}"
            f"-ts_{self.limit_size}"
            f"-fq_{self.freq}"
...
class ExpAdapting:
    def __init__(self, config: ExpConfig):
        ...
        checkpoint_path = self.config.checkpoint_path.replace(
            "{SEED}", str(self.config.seed)
        )
        assert Path(checkpoint_path).exists(), "Checkpoint file not found"
    ...
    def _load_data(self):
        ...
        for dataset, settings in DATASET_CONFIG.items():
            if dataset != self.config.dataset:
                continue
            datasets[dataset] = {}
            for flag in ["train", "vali", "test"]:
                ...
                datasets[dataset][flag] = data_loader
            ...
            print(
                f"Dataset: {dataset}, "
                f"Train size: {datasets[dataset]['train_size']}, "
                f"Vali size: {datasets[dataset]['vali_size']}, "
                f"Test size: {datasets[dataset]['test_size']}"
            )
    ...
    def train(self, setting: str):
        ...
        with log_file_path.open("a", encoding="utf-8") as f:
            for epoch in range(self.config.train_epochs):
                ...
                seq_len = DATASET_CONFIG[self.config.dataset]["seq_len"]
                datasets = self.datasets[self.config.dataset]
                for batch_x, label in tqdm(datasets["train"]):
                    self.model.train()
                    ...
                    outputs = self.model.forward(features, self.config.dataset)

                    loss = self.criteria[self.config.dataset](outputs, label)
                ...
                vali_loss, val_metrics_dict = self.vali(
                    self.config.dataset, datasets["vali"]
                )
                test_loss, test_metrics_dict = self.vali(
                    self.config.dataset, datasets["test"]
                )
                vali_results[self.config.dataset] = val_metrics_dict
                test_results[self.config.dataset] = test_metrics_dict
                vali_summary = (
                    f"Validation on {self.config.dataset} --- Loss: {vali_loss:.5f}, "
                ...
                test_summary = (
                    f"Test on {self.config.dataset} --- Loss: {test_loss:.5f}, "
    ...
    def test(self, setting: str):
        ...
        file_path = folder_path / "result_classification_attention_finetune.txt"
